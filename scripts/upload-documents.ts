 ...
 * This script uploads system-prompt.txt, resume.pdf, and cover-letter.pdf from data/ to R2
 */
...
function checkAllFilesExist(): { allPresent: boolean; missing: FileToUpload[] } {
...
function uploadToR2(fileName: string, r2Key: string): void {
  ...
  try {
    // Use wrangler to upload the file to R2 (remote/production)
    execSync(
...
function main() {
  console.log('Cloudflare R2 Document Upload Tool\n');
  console.log('==================================\n');
  
  // Check if data directory exists
  if (!existsSync(DATA_DIR)) {
    console.error('❌ Error: The data/ directory does not exist.\n');
    console.log('Please create the data/ directory in the project root and add your documents.\n');
    process.exit(1);
  }
  ...
  if (!allPresent) {
    ...
    console.error('Please add all required files to the data/ directory before running this script.');
    console.error('\nExpected files:');
    for (const file of FILES_TO_UPLOAD) {
      console.error(`   - data/${file.fileName}`);
    }
    console.error('\nSee system-prompt.example.txt for a template to create your system-prompt.txt\n');
